...
### Step 5 - Preprocess the ancillary data :
  ## 5.1 - preprocess the DEM : mosaic the various tiles covering the ROI (if relevant), and then extract the ROI
  ## 5.2 - preprocess the Sentinel 2 image(s) : mosaic the various images covering the ROI (if relevant), and then extract the ROI
#### Step 6 - Prepare the ancillary data for the classification : 
  ...
  ## 6.2 - extract textural indices from the Spot6/7 panchromatic image : 
  ## 6.3 - extract radiometric indices from the Spot6/7 pansharpened image : 
  ## 6.4 - extract radiometric indices from the S2 image : 
  ## 6.5 - Split the bands of the Spot6/7 image
...
# Radiometric indices to compute for the Spot6/7 image
radiometric_indices_list_spot67<-c("Vegetation:NDVI","Vegetation:TNDVI","Vegetation:RVI","Vegetation:SAVI","Vegetation:TSAVI","Vegetation:MSAVI","Vegetation:MSAVI2","Vegetation:GEMI","Vegetation:IPVI","Water:NDWI2","Water:NDTI","Soil:RI","Soil:CI","Soil:BI","Soil:BI2")
# Radiometric indices to compute for the Sentinel 2 image. There are more indices for S2 than for spot6/7 since there is a MIR band (B11) that does not exist in Spot
radiometric_indices_list_sentinel2<-c("Vegetation:NDVI","Vegetation:TNDVI","Vegetation:RVI","Vegetation:SAVI","Vegetation:IPVI","Water:NDWI","Water:NDWI2","Water:MNDWI","Water:NDPI","Water:NDTI","Soil:RI","Soil:CI","Soil:BI","Soil:BI2")
...
# Path to the folder where the datasets extracted from the Spot6/7 will be stored
path_to_spot67_preprocessed_folder=gsub("raw","processed",path_to_spot67_raw_folder) 
# Path to the output Spot6/7 multispectral image
path_to_spot67_preprocessed_ms=file.path(path_to_spot67_preprocessed_folder,"MS.TIF")
# Path to the output Spot6/7 panchromatic image
path_to_spot67_preprocessed_pan=file.path(path_to_spot67_preprocessed_folder,"PAN.TIF")
# Path to the output Spot6/7 pansharpened image
path_to_spot67_preprocessed_pansharpen=file.path(path_to_spot67_preprocessed_folder,"PANSHARPEN.TIF")
# Path to Sentinel 2 raw data
path_to_sentinel2_raw_folder<-"HR_Sentinel2/raw_data"
# Path to Sentinel 2 processed data
path_to_sentinel2_preprocessed_folder<-"HR_Sentinel2/processed_data"
# Path to DEM raw data
path_to_dem_raw_folder="DEM_SRTM/raw_data"
# Path to DEM processed data
path_to_dem_preprocessed_folder="DEM_SRTM/processed_data"
...
dir.create(path_to_spot67_preprocessed_folder)
...
for (i in 1:length(products_to_preprocess)){
  ...
  res<-orthorectify_spot67(PAN_toc_path,PAN_orthorectified_path,path_to_otbApplications_folder,path_to_dem_raw_folder)
  cat(res)
  ## MS 
  MS_orthorectified_path=file.path(spot67_preprocessing_output_folder_path,"MS_ORTHO.TIF")
  res<-orthorectify_spot67(MS_toc_path,MS_orthorectified_path,path_to_otbApplications_folder,path_to_dem_raw_folder)
  cat(res)
...
# Create directory where output datasets will be stored
path_to_dem_output_preprocessed_folder=gsub("raw","processed",path_to_dem_raw_folder)
...
if(length(products_to_preprocess)>1){
  res<-mosaic_and_extract_roi(products_to_preprocess,path_to_dem_output_preprocessed_folder,dem_output_path_file,path_to_roi_vector,path_to_otbApplications_folder,path_to_dem_raw_folder)
  } else {
...
# Create directory where output datasets will be stored
dir.create(path_to_sentinel2_preprocessed_folder)
...
for (i in 1:length(patterns)){
  ...
  if(length(products_to_preprocess)>1){
  res<-mosaic_and_extract_roi(paths_images_to_mosaic,path_to_sentinel2_preprocessed_folder,path_to_output_mosaiced_image,path_to_roi_vector,path_to_otbApplications_folder,path_to_dem_raw_folder)
  } else {
...
##############################################################
#### 6.4 - extract radiometric indices from the Sentinel 2 image ####
##############################################################

#### First create a temporary dataset with the bands of interest
path_to_s2_blue_band<-file.path(path_to_processing_folder,path_to_sentinel2_preprocessed_folder,"B02.TIF")
path_to_s2_green_band<-file.path(path_to_processing_folder,path_to_sentinel2_preprocessed_folder,"B03.TIF")
path_to_s2_red_band<-file.path(path_to_processing_folder,path_to_sentinel2_preprocessed_folder,"B04.TIF")
path_to_s2_nir_band<-file.path(path_to_processing_folder,path_to_sentinel2_preprocessed_folder,"B08.TIF")
path_to_s2_mir_band<-file.path(path_to_processing_folder,path_to_sentinel2_preprocessed_folder,"B11.TIF")  

path_to_s2_composite_tif<-file.path(path_to_processing_folder,path_to_sentinel2_preprocessed_folder,"S2_composite.TIF")

## B11 (MIR band) is 20 x 20  m resolution (while the 4 others are 10 m x 10 m resolution), hence it has to be cut and resized before being concatenated with the other bands
# modify extent
b11<-raster::brick(path_to_s2_mir_band)
b2<-raster::raster(path_to_s2_red_band)
extent(b11)=extent(b2)
writeRaster(b11,file.path(path_to_sentinel2_preprocessed_folder,"B11_resample.tif"))
# modify resolution
system(paste0('gdal_translate -tr 10 10 "',file.path(path_to_sentinel2_preprocessed_folder,"B11_resample.tif"),'" "',file.path(path_to_sentinel2_preprocessed_folder,"B11_resample_10m.tif"),'"'))

path_to_s2_mir_band<-file.path(path_to_sentinel2_preprocessed_folder,"B11_resample_10m.tif")

# Concatenate image
otb_appli<-paste0(file.path(path_to_otbApplications_folder,"otbcli_ConcatenateImages")," -il ",paste(path_to_s2_blue_band,path_to_s2_green_band,path_to_s2_red_band,path_to_s2_nir_band,path_to_s2_mir_band,sep=" ")," -out ",path_to_s2_composite_tif)
system(otb_appli)

# remove temporary files
file.remove(c(file.path(path_to_sentinel2_preprocessed_folder,"B11_resample.tif"),file.path(path_to_sentinel2_preprocessed_folder,"B11_resample_10m.tif")))

### Extract indices
  for (i in 1:length(radiometric_indices_list_sentinel2)){
  
  indice<-sub(".*:","",radiometric_indices_list_sentinel2[i])
  path_to_output_indice<-file.path(path_to_processing_folder,path_to_sentinel2_preprocessed_folder,paste0(indice,".TIF"))
  
  otb_appli<-paste0(file.path(path_to_otbApplications_folder,"otbcli_RadiometricIndices")," -in ",path_to_s2_composite_tif," -channels.blue 1 -channels.green 2 -channels.red 3 -channels.nir 4 -channels.mir 5 -list ",radiometric_indices_list_sentinel2[i]," -out ",path_to_output_indice)
  system(otb_appli)
}
